...
void ImageVisual::DoSetProperty(Property::Index index, const Property::Value& value)
{
  switch(index)
  {
    ...
    case Toolkit::ImageVisual::Property::ALPHA_MASK_URL:
    {
      ...
      {
        AllocateMaskData();
        mMaskingData->mAlphaMaskUrl = ToStdString(alphaUrl);
...
void ImageVisual::LoadTexture(TextureSet& textures, const Dali::ImageDimensions& size, TextureManager::ReloadPolicy forceReload)
{
  TextureManager& textureManager = mFactoryCache.GetTextureManager();
...
void ImageVisual::InitializeRenderer()
{
  ...
  if(!mTextures)
  {
    ...
    {
      LoadTexture(mTextures, mUseSynchronousSizing ? mLastRequiredSize : mDesiredSize, TextureManager::ReloadPolicy::CACHED);
    }
    else
    {
...
void ImageVisual::DoCreatePropertyMap(Property::Map& map) const
{
  ...
  {
    map.Insert(Toolkit::ImageVisual::Property::ALPHA_MASK_URL, ToDaliString(mMaskingData->mAlphaMaskUrl.GetUrl()));
...
Shader ImageVisual::GenerateShader() const
{
  ...
  else
  {
    ...
    if(useNativeImage)
    {
      ...
      shader = Shader::New(ToDaliStringView(vertexShaderView), ToDaliStringView(fragmentShaderView), mImpl->GetCustomShaderAt(0)->mHints, ToDaliStringView(mImpl->GetCustomShaderAt(0)->mName));
    }
    else
    {
      shader = Shader::New(ToDaliStringView(vertexShaderView), ToDaliStringView(fragmentShaderView), mImpl->GetCustomShaderAt(0)->mHints, ToDaliStringView(mImpl->GetCustomShaderAt(0)->mName));
