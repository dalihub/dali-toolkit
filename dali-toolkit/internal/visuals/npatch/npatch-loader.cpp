...
NPatchData::NPatchDataId NPatchLoader::Load(TextureManager& textureManager, TextureUploadObserver* textureObserver, const VisualUrl& url, const Dali::Extents& border, bool& preMultiplyOnLoad, bool synchronousLoading)
{
  ...
  else // if NOT_STARTED or LOADING or LOAD_FAILED, try to reload.
  {
    ...
    PixelBuffer pixelBuffer = textureManager.LoadPixelBuffer(url, Dali::ImageDimensions(), SamplingMode::BOX_THEN_LINEAR, synchronousLoading, data.Get(), true, preMultiplyOnLoading);
