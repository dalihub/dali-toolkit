        ...
        if(DALI_LIKELY(mGradient))
        {
          ...
          if(mImpl->mRenderer)
          {
            ...
            if(mGradientType == Type::CONIC)
            {
              ...
              mImpl->mRenderer.RegisterProperty(UNIFORM_START_ANGLE_NAME, gradient->GetStartAngle().radian);
            }
          }
...
void GradientVisual::OnInitialize()
{
  ...
  mImpl->mRenderer.RegisterUniqueProperty(UNIFORM_TEXTURE_COORDINATE_SCALE_FACTOR_NAME, 1.0f);
...
void GradientVisual::ApplyLookupTexture()
{
  ...
  if(DALI_LIKELY(mGradient) && mImpl->mRenderer)
  {
    ...
    if(DALI_LIKELY(lookupTexture))
    {
      ...
      mImpl->mRenderer.RegisterProperty(UNIFORM_TEXTURE_COORDINATE_SCALE_FACTOR_NAME,
