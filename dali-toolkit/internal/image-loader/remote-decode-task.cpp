...
void RemoteDecodeTask::Process()
{
  ...
  if(!mLocalFilePath.empty())
  {
    if(mLoadYuvPlanes)
    {
      Dali::LoadImagePlanesFromFile(mLocalFilePath, pixelBuffers, mDesiredSize, mFittingMode, mSamplingMode, mOrientationCorrection);
    }
    else
    {
      pixelBuffer = Dali::LoadImageFromFile(mLocalFilePath, mDesiredSize, mFittingMode, mSamplingMode, mOrientationCorrection);
    }
  }
