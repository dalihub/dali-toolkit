                            ...
                            CallbackBase*                           callback)
{
  ...
  RemoteDecodeTaskPtr remoteDecodeTask = new RemoteDecodeTask(textureId, localFilePath, desiredSize, fittingMode, samplingMode, orientationCorrection, preMultiplyOnLoad, loadYuvPlanes, callback);
                                     ...
                                     const bool                                     loadYuvPlanes)
{
  ...
  {
    LoadingTaskPtr loadingTask = new LoadingTask(++mLoadTaskId, mTextureManager.GetEncodedImageBuffer(url), desiredSize, fittingMode, samplingMode, orientationCorrection, preMultiplyOnLoad, MakeCallback(this, &TextureAsyncLoadingHelper::AsyncLoadComplete));
    loadingTask->SetTextureId(textureId);
    Dali::AsyncTaskManager::Get().AddTask(loadingTask);
    return;
  }
  ...
  if(!url.IsLocalResource() && Dali::RemoteFileDownloadManager::IsAsyncDownloadSupported())
  {
    ...
    Dali::RemoteFileDownloadManager::Get().StartDownload(
      ...
      [asyncLoadingHelper, asyncTaskManager, textureId, desiredSize, fittingMode, samplingMode, orientationCorrection, preMultiplyOnLoad, loadYuvPlanes](bool success, const std::string& localFilePath) {
        ...
        if(!success || localFilePath.empty())
        {
          ...
          SubmitRemoteDecodeTask(asyncTaskManager, textureId, {}, desiredSize, fittingMode, samplingMode, orientationCorrection, preMultiplyOnLoad, loadYuvPlanes, MakeCallback(asyncLoadingHelper, &TextureAsyncLoadingHelper::RemoteDecodeComplete));
          return;
        }
        ...
        SubmitRemoteDecodeTask(asyncTaskManager, textureId, localFilePath, desiredSize, fittingMode, samplingMode, orientationCorrection, preMultiplyOnLoad, loadYuvPlanes, MakeCallback(asyncLoadingHelper, &TextureAsyncLoadingHelper::RemoteDecodeComplete));
      });
    return;
  }
  ...
  LoadingTaskPtr loadingTask = new LoadingTask(++mLoadTaskId, url, desiredSize, fittingMode, samplingMode, orientationCorrection, preMultiplyOnLoad, loadYuvPlanes, MakeCallback(this, &TextureAsyncLoadingHelper::AsyncLoadComplete));
  loadingTask->SetTextureId(textureId);
  Dali::AsyncTaskManager::Get().AddTask(loadingTask);
}
