...
void Popup::SetDisplayState(Toolkit::Popup::DisplayState displayState)
{
  ...
  if(display)
  {
    ...
    if(keyboardFocusManager)
    {
      ...
      if(Self().GetProperty<bool>(Actor::Property::FOCUSABLE))
      {
        // Setup the actgor to start focus from.
        Actor focusActor;
        if(mContent && mContent.GetProperty<bool>(Actor::Property::FOCUSABLE))
        {
          // If the content is focusable, move the focus to the content.
          focusActor = mContent;
        }
        else if(mFooter && mFooter.GetProperty<bool>(Actor::Property::FOCUSABLE))
        {
  ...
  else // Not visible.
  {
    ...
    if(mPreviousFocusedActor && mPreviousFocusedActor.GetProperty<bool>(Actor::Property::FOCUSABLE))
    {
      Dali::Toolkit::KeyboardFocusManager keyboardFocusManager = Dali::Toolkit::KeyboardFocusManager::Get();
      if(keyboardFocusManager)
      {
...
void Popup::AddFocusableChildrenRecursive(Actor parent, std::vector<Actor>& focusableActors)
{
  if(parent)
  {
    ...
    if(parent.GetProperty<bool>(Actor::Property::FOCUSABLE) || layoutControl)
    {
...
void Popup::SetupTouch()
{
  if(!mTouchTransparent)
  {
    ...
    mBacking.TouchEventSignal().Connect(this, &Popup::OnBackingTouched);
    mPopupBackgroundImage.TouchEventSignal().Connect(this, &Popup::OnDialogTouched);
    mPopupLayout.TouchEventSignal().Connect(this, &Popup::OnDialogTouched);
  ...
  else
  {
    ...
    mBacking.TouchEventSignal().Disconnect(this, &Popup::OnBackingTouched);
    mPopupBackgroundImage.TouchEventSignal().Disconnect(this, &Popup::OnDialogTouched);
    mPopupLayout.TouchEventSignal().Disconnect(this, &Popup::OnDialogTouched);
