...
{
  GridLayoutPtr layout( new GridLayout() );
  return layout;
}
...
void GridLayout::DoInitialize()
{
}

void GridLayout::DoRegisterChildProperties( const std::string& containerType )
{
  ...
  if( typeInfo )
  {
    ...
    if( std::find( indices.Begin(), indices.End(), Toolkit::Grid
  Layout::ChildProperty::WEIGHT ) == indices.End() )
    {
      ChildPropertyRegistration( typeInfo.GetName(), "weight",
                                 Toolkit::Grid
                                Layout::ChildProperty::WEIGHT, Property::FLOAT );
    }
  }
}

void GridLayout::OnChildAdd( LayoutItem& child )
{
  auto owner = child.GetOwner();
  owner.SetProperty( Toolkit::GridLayout::ChildProperty::WEIGHT, 1.0f );
}
...
void GridLayout::OnMeasure( MeasureSpec widthMeasureSpec, MeasureSpec heightMeasureSpec )
{
#if defined(DEBUG_ENABLED)
  auto actor = Actor::DownCast(GetOwner());

  std::ostringstream oss;
  oss << "GridLayout::OnMeasure  ";
  if( actor )
  {
  ...
  DALI_LOG_INFO( gLogFilter, Debug::Concise, "GridLayout::OnMeasure widthSize(%d) \n", widthMeasureSpec.GetSize());
  ...
  for( unsigned int i=0; i<GetChildCount(); ++i )
  {
    ...
    if( childLayout )
    {
      ...
      DALI_LOG_INFO( gLogFilter, Debug::Verbose, "GridLayout::OnMeasure childWidth(%d)\n", MeasureSpec::IntType( childWidth ) );
...
void GridLayout::ForceUniformHeight( int count, MeasureSpec widthMeasureSpec )
{
...
void GridLayout::OnLayout( bool changed, LayoutLength left, LayoutLength top, LayoutLength right, LayoutLength bottom )
{
  auto owner = GetOwner();
