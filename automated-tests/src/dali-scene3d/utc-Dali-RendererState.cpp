...
#define DEPTH_FUNC_PAIR(x)                                      \
  {                                                             \
    Comparison::x << rs::DEPTH_FUNCTION_SHIFT, DepthFunction::x \
  }
...
{
  std::string error;
  auto        onError = [&](const std::string& e)
  { error = e; };
