...
struct TestDBusWrapper::MessageIterImpl : public TestDBusWrapper::MessageIter
{
  ...
  MessageIterImpl(MessagePtr msg, Element* elem, bool write)
  : msg(std::move(msg)),
    elem(elem),
    write(write)
  {
  ...
  MessageIterImpl(MessagePtr msg, Element* elem, bool write, std::list<Element>::iterator it)
  : msg(std::move(msg)),
    elem(elem),
    write(write),
    it(it)
...
struct TestDBusWrapper::MessageImpl : public TestDBusWrapper::Message
{
  ...
  : bus(std::move(bus)),
    ...
    reply(true),
    error(true),
    error_a(std::move(error_a)),
    error_b(std::move(error_b))
  {
  }
};
...
struct TestDBusWrapper::ProxyImpl : public TestDBusWrapper::Proxy
{
  ...
  ProxyImpl(std::string bus, std::string path, std::string interface)
  : bus(std::move(bus)),
    path(std::move(path)),
    interface(std::move(interface))
  {
...
{
  std::string bus, path;
  ObjectImpl(std::string bus, std::string path)
  : bus(std::move(bus)),
    path(std::move(path))
  {
  }
...
#define DEFINE_TYPE(name)                                               \
  TestDBusWrapper::name##Impl* TestDBusWrapper::get(const name##Ptr& v) \
  {                                                                     \
    return static_cast<name##Impl*>(v.get());                           \
  }
...
template<typename T>
void TestDBusWrapperAppendBasic(const TestDBusWrapper::MessageIterPtr& it, T src)
{
...
template<typename T>
bool TestDBusWrapperGetBasicAndNext(const TestDBusWrapper::MessageIterPtr& it, T& src)
{
  auto m = TestDBusWrapper::get(it);
  if(m->write) throw TestDBusWrapper::error{};
  ...
  const std::string& mpName, const MessagePtr& msg, MethodType type)
{
  ...
  if(it == mp.end())
  {
    ...
    else if(type == MethodType::Getter)
      mt = "MethodType::Getter";
    else if(type == MethodType::Setter)
      mt = "MethodType::Setter";
    else
      assert(0);
